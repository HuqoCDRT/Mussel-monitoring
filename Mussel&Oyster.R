...
g2=group_by(oybedmuss,bank,date,sample,sub_samp_area)
densoybedmuss=summarise(g2,length(sample)) #calculate number of ind for each sample


names(densoybedmuss)[names(densoybedmuss) == 'length(sample)'] <- 'n_mussels'
...
densoybedmuss$id<-paste(densoybedmuss$bank,densoybedmuss$date)
mean_Dmuss=aggregate(densoybedmuss$density,by=list(densoybedmuss$id),FUN=mean)
colnames(mean_Dmuss)<-c("id","mean mussel density")

sd_Dmuss=aggregate(densoybedmuss$density,by=list(densoybedmuss$id),FUN=sd)
...
table(densoybedmuss$bank,densoybedmuss$date)


sel_densoymuss <- densoybedmuss[densoybedmuss$bank=="LT01"|densoybedmuss$bank=="LT02"|densoybedmuss$bank=="LT07"|densoybedmuss$bank=="LT13"|
                                  densoybedmuss$bank=="NA23"|densoybedmuss$bank=="NA38",]


ggplot(sel_densoymuss, aes(x = date, y = density))+
  geom_boxplot()+
...
### Select years ###
LT01myears<-LT01m[LT01m$date=="2005"|LT01m$date=="2010"|LT01m$date=="2015"|LT01m$date=="2020",]
LT02myears<-LT02m[LT02m$date=="2005"|LT02m$date=="2010"|LT02m$date=="2015"|LT02m$date=="2020",]
LT07myears<-LT07m[LT07m$date=="2005"|LT07m$date=="2010"|LT07m$date=="2015"|LT07m$date=="2020",]
LT13myears<-LT13m[LT13m$date=="2005"|LT13m$date=="2010"|LT13m$date=="2015"|LT13m$date=="2020",]
NA23myears<-NA23m[NA23m$date=="2005"|NA23m$date=="2010"|NA23m$date=="2015"|NA23m$date=="2020",]
NA38myears<-NA38m[NA38m$date=="2005"|NA38m$date=="2010"|NA38m$date=="2015"|NA38m$date=="2020",]
...
pairwise.wilcox.test(LT01myears$density,LT01myears$date,p.adjust.method = "bonferroni")
pairwise.t.test(LT02myears$density,LT02myears$date,p.adjust.method = "bonferroni")
pairwise.wilcox.test(LT07myears$density,LT07myears$date,p.adjust.method = "bonferroni")
pairwise.wilcox.test(LT13myears$density,LT13myears$date,p.adjust.method = "bonferroni")
pairwise.wilcox.test(NA23myears$density,NA23myears$date,p.adjust.method = "bonferroni")
pairwise.wilcox.test(NA38myears$density,NA38myears$date,p.adjust.method = "bonferroni")
...
sel_densoymuss_years<- sel_densoymuss[sel_densoymuss$date=="2005"|sel_densoymuss$date=="2010"|sel_densoymuss$date=="2015"|sel_densoymuss$date=="2020",]
...
### For LT banks
ggplot(sel_densoymuss_years[grep("^LT", sel_densoymuss_years$bank), ], aes(x = date, y = density,fill=date))+
  geom_boxplot()+
  geom_signif(comparisons = list(c("2005", "2020"),c("2010","2020"),c("2015","2020")),
              map_signif_level=TRUE, y_position = c(1.1, 1,.9))+
  facet_wrap(.~bank, scales = "free")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  labs(title = "Mussel density over time on oyster beds LT01, LT02, LT07 and LT13",x="Date (year)",y="Density (ind/cm2)")+
...
g3=group_by(oybedoys,bank,date,sample,sub_samp_area)
densoybedoys=summarise(g3,length(sample)) #calculate number of ind for each sample



names(densoybedoys)[names(densoybedoys) == 'length(sample)'] <- 'n_oyster'



##add density column
densoybedoys=densoybedoys%>%
  mutate(density=n_oyster/sub_samp_area)


#Calculate mean density 
densoybedoys$id<-paste(densoybedoys$bank,densoybedoys$date)
mean_Doys=aggregate(densoybedoys$density,by=list(densoybedoys$id),FUN=mean)
colnames(mean_Doys)<-c("id","mean oyster density")

sd_Doys=aggregate(densoybedoys$density,by=list(densoybedoys$id),FUN=sd)




#### 2.a Plot of some banks ####
table(densoybedoys$bank,densoybedoys$date)


sel_densoyoy <- densoybedoys[densoybedoys$bank=="LT01"|densoybedoys$bank=="LT02"|densoybedoys$bank=="LT07"|densoybedoys$bank=="LT13"|
                               densoybedoys$bank=="NA23"|densoybedoys$bank=="NA38",]


ggplot(sel_densoyoy, aes(x = date, y = density))+
  geom_boxplot()+
  facet_wrap(.~bank, scales = "free")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
LT01o<-sel_densoyoy[sel_densoyoy$bank=="LT01",]
LT02o<-sel_densoyoy[sel_densoyoy$bank=="LT02",]
LT07o<-sel_densoyoy[sel_densoyoy$bank=="LT07",]
LT13o<-sel_densoyoy[sel_densoyoy$bank=="LT13",]
...
### Select years ###
LT01oyears<-LT01o[LT01o$date=="2005"|LT01o$date=="2010"|LT01o$date=="2015"|LT01o$date=="2020",]
LT02oyears<-LT02o[LT02o$date=="2005"|LT02o$date=="2010"|LT02o$date=="2015"|LT02o$date=="2020",]
LT07oyears<-LT07o[LT07o$date=="2005"|LT07o$date=="2010"|LT07o$date=="2015"|LT07o$date=="2020",]
LT13oyears<-LT13o[LT13o$date=="2005"|LT13o$date=="2010"|LT13o$date=="2015"|LT13o$date=="2020",]
NA23oyears<-NA23o[NA23o$date=="2005"|NA23o$date=="2010"|NA23o$date=="2015"|NA23o$date=="2020",]
NA38oyears<-NA38o[NA38o$date=="2005"|NA38o$date=="2010"|NA38o$date=="2015"|NA38o$date=="2020",]





pairwise.wilcox.test(LT01oyears$density,LT01oyears$date)
pairwise.wilcox.test(LT02oyears$density,LT02oyears$date)
pairwise.wilcox.test(LT07oyears$density,LT07oyears$date)
pairwise.wilcox.test(LT13oyears$density,LT13oyears$date)
pairwise.wilcox.test(NA23oyears$density,NA23oyears$date)
pairwise.wilcox.test(NA38oyears$density,NA38oyears$date)
...
sel_densoyoy_years<- sel_densoyoy[sel_densoyoy$date=="2005"|sel_densoyoy$date=="2010"|sel_densoyoy$date=="2015"|sel_densoyoy$date=="2020",]
...
### For LT banks
ggplot(sel_densoyoy_years[grep("^LT", sel_densoyoy_years$bank), ], aes(x = date, y = density,fill=date))+
  geom_boxplot()+
  geom_signif(comparisons = list(c("2005", "2020"),c("2010","2020"),c("2015","2020")),
              map_signif_level=TRUE, y_position = c(.8, .7,.6))+
  facet_wrap(.~bank, scales = "free")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  labs(title = "Oyster density over time on oyster beds LT01, LT02, LT07 and LT13",x="Date (year)",y="Density (ind/cm2)")+
  scale_fill_brewer(palette="Set3")
